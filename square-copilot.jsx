...
const RiskAnalyzer = {
  ...
  calculateStockoutRisk(item, dailyVelocity) {
    if (dailyVelocity === 0)
      return { risk: "low", daysUntilStockout: 999, severity: 0 };
  ...
  calculateOverstockRisk(item, dailyVelocity) {
    if (dailyVelocity === 0) {
      return {
        ...
        daysOfStock: 999,
        severity: 80,
        tiedUpCapital: item.currentStock * item.costPerUnit,
      };
    }
  ...
  generateRecommendation(item, stockoutRisk, overstockRisk, dailyVelocity) {
    ...
    if (stockoutRisk.risk === "critical" || stockoutRisk.risk === "high") {
      ...
      recommendations.push({
        ...
        reason: `Only ${Math.floor(
          stockoutRisk.daysUntilStockout
        )} days of stock remaining`,
        impact: `Prevent ${Math.round(
          dailyVelocity * item.pricePerUnit * stockoutRisk.daysUntilStockout
        )} in lost revenue`,
      });
    ...
    if (overstockRisk.risk === "high") {
      if (overstockRisk.daysOfStock > 60) {
        recommendations.push({
          ...
          impact: `Free up $${overstockRisk.tiedUpCapital.toFixed(
            2
          )} in tied capital`,
        });
      } else {
  ...
  analyzeInventory(inventory, salesData) {
    return inventory.map((item) => {
      ...
      const overallRisk = Math.max(
        stockoutRisk.severity,
        overstockRisk.severity
      );
...
export default function SquareSellerCopilot() {
  ...
  const filteredInventory = inventory.filter((item) => {
    ...
    if (filterRisk === "stockout")
      return (
        item.stockoutRisk.risk === "critical" ||
        item.stockoutRisk.risk === "high"
      );
    if (filterRisk === "overstock") return item.overstockRisk.risk === "high";
    return true;
  });

  const criticalAlerts = inventory.filter(
    (item) =>
      item.stockoutRisk.risk === "critical" ||
      (item.overstockRisk.risk === "high" &&
        item.overstockRisk.daysOfStock > 60)
  ).length;

  const totalCapitalTied = inventory.reduce(
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
              ...
              >
                <div className="p-5">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                        ...
                        <div>
                          ...
                          <p className="text-lg font-bold text-slate-900">
                            {item.dailyVelocity.toFixed(1)} units/day
                          </p>
                        </div>
                        <div>
                          <p className="text-xs text-slate-500 mb-1">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex-1 bg-slate-100 rounded-full h-3 overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.min(item.overallRisk, 100)}%`,
                            }}
                          />
                        </div>
                        <span className="text-sm font-medium text-slate-600">
                    ...
                    <ChevronRight
                      className={`w-6 h-6 text-slate-400 transition-transform ml-4 ${
                          ...
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              ...
                              <span className="font-semibold">
                                {Math.floor(
                                  item.stockoutRisk.daysUntilStockout
                                )}{" "}
                                days
                              </span>
                            </div>
                        ...
                        >
                          ...
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              ...
                              <span className="font-semibold">
                                {Math.floor(item.overstockRisk.daysOfStock)}{" "}
                                days
                              </span>
                            </div>
                            <div className="flex justify-between">
                            ...
                            <div className="flex justify-between">
                              ...
                              <span className="font-semibold">
                                ${item.overstockRisk.tiedUpCapital.toFixed(2)}
                              </span>
                            </div>
                          </div>
                        </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 text-center text-sm text-slate-500">
        <p>
          Square Seller Copilot • Inventory Risk Predictor • Powered by Square
          API
        </p>
        <p className="mt-1">
          Currently using {apiToken ? "Live" : "Demo"} data
        </p>
      </div>
    </div>
  );
